...
ExprReturn visit_literal (AST *ast) {
    ...
    ExprReturn ret = {};
	printm("<<< literal\n");
	ret.type = ast->expr.literal.type;
    return ret;
}
...
ExprReturn visit_unary_minus (AST *ast) {
	...
	if (expr.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.unary_minus.token->filename, ast->expr.unary_minus.token->line, ast->expr.unary_minus.token->column);
		printf("Não pode usar uma STRING na operação UNARY_MINUS\n");
		COLOR_RESET
	}
...
ExprReturn visit_add (AST *ast) {
	...
	if (left.type == TYPE_STRING || right.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar uma STRING na operação ADD\n");
		COLOR_RESET
	}
...
ExprReturn visit_sub (AST *ast) {
	...
	if (left.type == TYPE_STRING || right.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar uma STRING na operação SUB\n");
		COLOR_RESET
	}
...
ExprReturn visit_mul (AST *ast) {
	...
	if (left.type == TYPE_STRING || right.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar uma STRING na operação MUL\n");
		COLOR_RESET
	}
...
ExprReturn visit_div (AST *ast) {
	...
	if (left.type == TYPE_STRING || right.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar uma STRING na operação DIV\n");
		COLOR_RESET
	}
...
ExprReturn visit_mod (AST *ast) {
	...
	if (left.type == TYPE_FLOAT || right.type == TYPE_FLOAT) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar um FLOAT na operação MOD\n");
		COLOR_RESET
	} else if (left.type == TYPE_STRING || right.type == TYPE_STRING) {
		COLOR_RED
		printf("%s : (Line: %d, Col: %d) : Erro Semântico\n", ast->expr.binary_expr.token->filename, ast->expr.binary_expr.token->line, ast->expr.binary_expr.token->column);
		printf("Não pode usar uma STRING na operação MOD\n");
		COLOR_RESET
	}
